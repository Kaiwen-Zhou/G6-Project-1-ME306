 ...
 *
 * Supported commands:
 *   G01 or G1  - relative linear movement using X, Y and F
 *   G28        - request homing
 *   M999       - request recovery from the FAULT state
 *
 * G90 and G91 are deliberately not parsed. The project always uses relative
 * positioning, as required by Lecture 4.
 *
...
{
public:
    explicit GCodeParser(const GCodeSafetyLimits& safetyLimits);
