...
{
    return isFiniteFloat(limits.xMinimumMm) &&
           isFiniteFloat(limits.xMaximumMm) &&
           isFiniteFloat(limits.yMinimumMm) &&
           isFiniteFloat(limits.yMaximumMm) &&
           isFiniteFloat(limits.maximumFeedrateMmPerMinute) &&
           limits.xMinimumMm <= limits.xMaximumMm &&
           limits.yMinimumMm <= limits.yMaximumMm &&
           limits.maximumFeedrateMmPerMinute > 0.0f;
}

}  // namespace

GCodeParser::GCodeParser(const GCodeSafetyLimits& safetyLimits)
    : safetyLimits_(safetyLimits),
...
GCodeInputResult GCodeParser::processCharacter(char character,
                                               float currentXMm,
                                               float currentYMm)
{
    if (character == '\n' && ignoreNextLineFeed_)
...
GCodeParseResult GCodeParser::parseLine(const char* line,
                                        float currentXMm,
                                        float currentYMm) const
{
    if (line == nullptr)
    {
    ...
    while (true)
    {
        ...
        if (label == 'G' || label == 'M')
        {
            ...
            const CommandNumberResult commandNumber =
                parseCommandNumber(cursor);
    ...
    if (commandType == GCodeCommandType::HOME ||
        commandType == GCodeCommandType::RESET_FAULT)
    {
        if (parameterSeen)
        {
    ...
    if (!isFiniteFloat(feedrateMmPerMinute) ||
        feedrateMmPerMinute <= 0.0f)
    {
    ...
    if (feedrateMmPerMinute >
        safetyLimits_.maximumFeedrateMmPerMinute)
    {
        feedrateMmPerMinute =
            safetyLimits_.maximumFeedrateMmPerMinute;
    ...
    if (hasMovement)
    {
        ...
        const float targetXMm = currentXMm + xOffsetMm;
        const float targetYMm = currentYMm + yOffsetMm;

        if (!isFiniteFloat(targetXMm) ||
            !isFiniteFloat(targetYMm) ||
            targetXMm < safetyLimits_.xMinimumMm ||
            targetXMm > safetyLimits_.xMaximumMm ||
            targetYMm < safetyLimits_.yMinimumMm ||
...
void GCodeParser::commitCommand(const GCodeCommand& command)
{
    switch (command.type)
    {
        ...
        case GCodeCommandType::LINEAR_MOVE:
            if (isFiniteFloat(command.feedrateMmPerMinute) &&
                command.feedrateMmPerMinute > 0.0f)
            {
                lastFeedrateMmPerMinute_ =
                    command.feedrateMmPerMinute;

                if (lastFeedrateMmPerMinute_ >
                    safetyLimits_.maximumFeedrateMmPerMinute)
                {
                    lastFeedrateMmPerMinute_ =
                        safetyLimits_.maximumFeedrateMmPerMinute;
...
const char* GCodeParser::errorMessage(GCodeParseError error)
{
    switch (error)
    {
        ...
        case GCodeParseError::POSITION_OUT_OF_RANGE:
            return "The relative move would exceed the configured workspace.";
